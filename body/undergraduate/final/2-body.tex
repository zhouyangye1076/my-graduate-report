...
migrate\_secret 块负责转移 secret 数据在分层存储结构中的位置。处理器内存采用 line fill buffer、cache、memory
...
随机产生一个数据流依赖序列，然后开始根据这个依赖序列依次进行指令生成。
...
但是排布位置在 trigger 指令地址的基础上随机偏移一段距离。变体三为多条指令，此时讨论指令对齐没有意义，
...
该种类型的漏洞，生成框架在构造地址时会将地址的高位随机替换为一个随机数，被替换的高位在 0 位到 28 位的范围中随机选择。\par
...
为了防止 li 指令长度不可控，造成后续生成的指令不对齐，当地址被随机数高位覆盖时，只能使用方法二获得地址。
...
因为差分测试的时候本体和变体都会执行该操作，所以虽然访问了 secret 但是不会造成执行时间的差异。\par
    ...
    一个 cacheline 的数据首先会被缓存到 line fill buffer，等一个 cacheline 读取完毕后，
    line fill buffer 中的数据会被写入 cacheline。因此在 memory 和 cache 之间还有一个 line fill buffer 的存储层次。
...
在构造基本块的时候，生成框架希望基本块产生的执行行为尽可能的极端，可以让流水线的执行行为尽可能地复杂、极端。
...
当 decode group 重新执行这部分代码时，因为 s0 的值在本体变体都是 101，所以访问地址  channel\_page\_base\_0 + 96 + 0x5c8 和 channel\_page\_base\_0 + 96 + 0x5e8 。
...
另外也包括一些原理不明的 PoC 程序有待后续进一步地分析归类，诸如 rsb、btb、bim 等前端部件也有被作为侧信道的可能。
