...
\textbf{RETURN+JALR 的组合}利用了 return 指令本身也是 jalr 指令的特性进行工作。这说明了 return 指令
...
\begin{table}[h!]
    ...
    \resizebox{1.0\linewidth}{!}{
        \begin{tabular}{|l|l|l|l|} 
            \hline
            \textbf{瞬态窗口类别} & \textbf{预测部件} & \textbf{能否瞬态攻击} & \textbf{原理}\\
            \hline
            JALR+NONE               &   btb &   是  &   btb 默认预测错误      \\
            JALR+CALL/RETURN/JALR   &   btb &   是  &   jalr训练btb导致预测错误 \\
            JALR+BRANCH/JMP         &   btb &   是  &   jmp/branch训练btb导致预测错误 \\
            RETURN+CALL             &   rsb &   是  &   call训练rsb导致预测错误 \\
            RETURN+JALR             &   btb &   是  &   jalr训练btb导致预测错误 \\
            BRANCH+NONE             &   bim &   是  &   bim初始化值预测错误 \\
            BRANCH+BRANCH           &   bim &   是  &   branch训练bim导致预测错误 \\
            STORE\_PAGE\_FAULT      &   无  &   是  &   异常、权限延迟检测  \\
            STORE\_ACCESS\_FAULT    &   无  &   是  &   异常、权限延迟检测  \\
            STORE\_MISALIGN         &   无  &   否  &   异常、权限延迟检测  \\
            LOAD\_PAGE\_FAULT       &   无  &   是  &   异常、权限延迟检测  \\
            LOAD\_ACCESS\_FAULT     &   无  &   是  &   异常、权限延迟检测  \\
            LOAD\_MISALIGN          &   无  &   否  &   异常、权限延迟检测  \\
            V4                      &   lsu &   是  &   内存未修改预测错误  \\
            V4+AMO/STORE/return     &   lsu &   是  &   未知               \\
            \hline
        \end{tabular}
    }
    \end{center}
