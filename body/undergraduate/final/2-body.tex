...
随着 CPU 复杂度的不断提高和瞬态攻击研究的不断推进，更多微架构状态和侧信道被用应于瞬态执行攻击。\par
...
\textbf{场景二：} trigger 指令是控制流指令，如 branch、jmp ，
...
执行物理内存切换。物理内存种的指令切换为下一个 train group，继续训练，直到最后切换为
...
其中用户态的代码页和数据页都需要一个额外的内核态的页表项进行映射，
...
阶段 2 如图\ref{paper:stage2-test}所示，在阶段 1 程序的基础上产生阶段二的代码块配置，
