...
\section{RISCV处理器瞬态执行漏洞测试程序生成背景}
...
该变体使用 arbitrary 块生成训练，该模块我们在后续进行介绍。\par
...
对于 train 块的变体 3，我们可以使用 arbitrary 块产生多条随机的不对齐的训练指令，用于 trigger 块的训练，\par
...
将寄存器的值和 0x7f8 计算，然后和和 la 得到的基地值相加。
...
本实验在 16 物理核，3GHz主频，128G内存的 AMD 5950X 服务器上进行测评。\par
...
依次展示了 V1、V2、V3 找到的瞬态窗口种类、该类瞬态窗口的个数、该类瞬态窗口在大类中占比等信息。\par
